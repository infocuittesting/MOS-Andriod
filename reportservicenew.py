...
#-----------------------Room based Report-------------------------------------------------#
    ...
    final_request=hkrequest+fdrequest+fbrequest
    c = defaultdict(int)
...
#-------------------------department_report---------------------------#
    fbrequest1 = json.loads(dbget("select hotel_department.dept_name as department ,count(*) from fb_requests \
                                  join foodandbeverage_items on foodandbeverage_items.fbitem_id=fb_requests.fbitem_id\
    ...
    dept_final=hk_request1+fbrequest1+fd_request1
#----------------------------remainder_count_report---------------------------------------#  
    ...
    remainder_report=hkrequest2+fb_request2+fd_request2
...
#-----------------------Escalation_based_report---------------------#
    ...
    (select count(escalation_count) as escalation_one from fd_requests where escalation_count=1),\
    (select count(escalation_count) as escalation_two from fd_requests where escalation_count=2) from fd_requests\
      join frontdesk_items on frontdesk_items.fditem_id = fd_requests.fditem_id\
      join fdcategory on fdcategory.fdcategory_id = frontdesk_items.fdcategory_id\
      where date(request_time) between '"+d['datefrom']+"' and '"+d['dateto']+"' \
      and fd_requests.business_id ='"+d['business_id']+"' group by fdcategory.fdcategory_name"))

    esclation=fd_request3+fb_request3+hkrequest3
